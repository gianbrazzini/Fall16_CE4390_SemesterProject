...
struct sr_rt *sr_longest_prefix_match(struct sr_instance *sr, struct in_addr addr)
{
	...
	while (current_entry)
	{
		if ((current_entry->dest.s_addr & current_entry->mask.s_addr) == (addr.s_addr & current_entry->mask.s_addr))
		{
			if (prefix_len < current_entry->mask.s_addr)
			{
				prefix_len = current_entry->mask.s_addr;
				prefix_entry = current_entry;
			}
		}
...
{
	uint8_t *icmp_header;
	icmp_header = (unint8_t*)(ip_header);
